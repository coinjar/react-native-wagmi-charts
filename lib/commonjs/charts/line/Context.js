...
function LineChartProvider({
  ...
  const domain = React.useMemo(() => (0, _utils.getDomain)(Array.isArray(trusteeData) ? trusteeData : Object.values(trusteeData)[0]), [trusteeData]);
  const contextValue = React.useMemo(() => {
    var _yRange$min, _yRange$max;

    const values = (0, _utils.lineChartDataPropToArray)(trusteeData).map(({
      value
    }) => value);
    return {
      currentX,
